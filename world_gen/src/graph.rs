...
impl WorldGraph {
    pub fn new(width: f64, height: f64) -> Self {
        Self { width, height, centers: vec![], corners: vec![], edges: vec![] }
    }

    /// Nombre de centres réels (hors points fantômes).
    pub fn real_center_count(&self) -> usize {
        self.centers.iter().filter(|c| !c.is_ghost).count()
    }
}
